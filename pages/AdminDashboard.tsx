...
const BanUserForm = () => {
  ...
  const handleBanUser = async () => {
    try {
      const resp = await axios.post('http://192.168.0.100:3001/admin/ban-user', {
        user_id: userId,
...
export default function AdminDashboard() {
  ...
  const fetchComments = async () => {
    try {
      const resp = await axios.get('http://192.168.0.100:3001/admin/comments');
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      await axios.delete(`http://192.168.0.100:3001/admin/comments/${commentId}`);
